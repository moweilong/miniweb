...
VERSION_PACKAGE=github.com/moweilong/miniweb/pkg/version
...
build: tidy # 编译源码，依赖 tidy 目标自动添加/移除依赖包.
	@go build -v -ldflags "$(GO_LDFLAGS)" -o $(OUTPUT_DIR)/miniweb $(ROOT_DIR)/cmd/miniweb/main.go
...
ca: ## 生成 CA 文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/ca.key -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=Guangdong/L=GuangZhou/O=devops/OU=it/CN=127.0.0.1/emailAddress=kalandramo@gmail.com" # 2. 生成请求文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=Guangdong/L=GuangZhou/O=serverdevops/OU=serverit/CN=127.0.0.1/emailAddress=kalandramo@gmail.com" # 6. 生成服务端向 CA 申请签名的 CSR
